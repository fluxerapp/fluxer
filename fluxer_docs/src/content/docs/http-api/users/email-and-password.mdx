...
Changing the email address or the password on an account takes several steps. Fluxer opens a ticket, emails a code, and every later step presents that ticket together with the codes and proofs it has collected. An account whose address the mail provider rejected uses [bounced email recovery](#bounced-email-recovery) instead.
...
An issued code has only uppercase letters, digits, and one hyphen. A client that accepts free-form input uppercases the value and preserves the separator before submitting it.
:::
...
| original_email<sup>2</sup> | ?string | The address currently on the account, or null when the account has none |
...
<sup>3</sup> Populated exactly when `require_original` is false. [Request new email](#request-new-email) and [Verify new email](#verify-new-email) require it
...
The replacement session [Complete password change](#complete-password-change) hands back after the password is written.
...
<sup>1</sup> The same operation destroys the token the client authenticated with, so adopt this value before issuing another request
...
The password and its change timestamp are replaced and the ticket is marked completed. Every outstanding password reset token is deleted. Fluxer deletes and terminates every other authentication session on the Gateway first, then emits [Auth Session Change](/gateway/events/#auth-session-change), then deletes and terminates the caller's own session. The returned replacement is then the account's only session. OAuth2 access tokens and refresh tokens are not revoked.
