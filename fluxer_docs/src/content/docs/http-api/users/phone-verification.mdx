...
| --- | --- | --- |
| 1 | The lookup could not be performed<sup>1</sup> | Refused with `INVALID_PHONE_NUMBER` |
...
<sup>1</sup> A phone denial uses the rate limit envelope with `code` set to `PHONE_RATE_LIMIT_EXCEEDED` rather than the `RATE_LIMITED` an ordinary route denial returns
...
Fluxer records an attempt against the phone attempt risk counters only once it reaches [provider lookup](#provider-lookup-verdicts).
...
| Status | Body | Condition |
| --- | --- | --- |
| 200 | [user](/http-api/users/#user-object) object | The escape ran, wholly or up to the per-request limit |
