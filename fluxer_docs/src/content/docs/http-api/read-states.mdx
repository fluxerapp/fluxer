...
<sup>1</sup> The value the last acknowledgement stored, incremented by the server as new mentions arrive. It is never recomputed from message history
...
When no entry sets `manual` and no entry has a `mention_count` above `0`, Fluxer applies the whole list through the batched path [Mark channels as read](#mark-channels-as-read) uses.
:::
