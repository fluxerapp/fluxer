...
Creates a key and returns an [Admin API key creation](#admin-api-key-creation-object) object that has the raw credential, with HTTP 200 rather than 201. Requires `admin_api_key:manage`.
...
<sup>1</sup> The ownership check runs before the ACL grant check, so a key belonging to another account returns 404 rather than 403
