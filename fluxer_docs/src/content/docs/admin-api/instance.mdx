...
| screen_share_delivery | [screen share delivery configuration](#screen-share-delivery-configuration-object) object | Adaptive screen share delivery rollout |
...
The instance rollout of adaptive screen share delivery. [Experiments](/http-api/experiments/) defines what a client resolves from it and what each arm does.
...
A client reads this rollout once, when a screen share starts, and holds that answer for the life of the share. Turning the rollout off leaves a share that is already running on the arm it started on.
...
`screen_share_delivery` takes every [screen share delivery configuration](#screen-share-delivery-configuration-object) field except `config_version`, each bound as documented there. Fluxer raises `config_version` by one on each request that supplies at least one of them. A section that is absent, or present with no field set, writes nothing and leaves `config_version` alone.
