...
The records are the same ones the public [Applications](/http-api/applications/) resource serves. Creation, deletion, renaming, bot provisioning, redirect URI editing, and credential rotation stay there.

:::note[Admin operations report credential metadata only]
The Admin application object reports whether a client secret or bot token hash is stored, when it was provisioned, and the non-secret bot token preview.
:::
...
| client_secret_created_at<sup>4</sup> | ?ISO8601 timestamp | When the client secret was provisioned |
