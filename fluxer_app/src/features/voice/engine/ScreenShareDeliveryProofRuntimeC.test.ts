...
describe('screen share delivery proof, runtime for P7 to P9', () => {
	it('runs every key, condition and codec policy', () => {
		...
		const outcome = runProofRuntimeStage(['P7', 'P8', 'P9']);
		reportProofStage('stage E P7 to P9', outcome);
		...
		expect(Object.keys(outcome.applied)).toEqual(
			expect.arrayContaining([...PROOF_RUNTIME_RULES, 'E4 connection-unmeasured']),
		);
	}, 30_000);
});
