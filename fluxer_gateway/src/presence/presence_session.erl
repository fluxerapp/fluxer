...
handle_session_connect(Request, Pid, State) ->
    ...
    Activities = normalize_activities(maps:get(<<"activities">>, Request, null)),
...
-spec refresh_existing_session(
    session_entry(), pid(), status(), boolean(), boolean(), activities(), pid() | undefined, state()
) -> {session_entry(), state()}.
    ...
    case maps:get(SessionId, Sessions, undefined) of
        ...
        Session ->
            ...
            Activities = normalize_activities(
                maps:get(<<"activities">>, Request, maps:get(activities, Session, null))
            ),
...
-spec session_presence_changed(session_entry(), status(), boolean(), boolean(), activities()) -> boolean().
session_presence_changed(Session, Status, Afk, Mobile, Activities) ->
    maps:get(status, Session) =/= Status orelse
        ...
        maps:get(activities, Session, null) =/= Activities.
...
-spec normalize_activities(term()) -> activities().
normalize_activities(null) ->
    null;
